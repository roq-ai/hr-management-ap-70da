...
import { useInterviewsCount } from 'lib/roq';

import { InterviewsListPage as ShowCaseList } from 'pages/interviews';

function DashboardPage() {
  const { session } = useSession();

  const interviewsStat = useInterviewsCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Interviews',
      result: interviewsStat?.data,
      isLoading: interviewsStat.isLoading,
